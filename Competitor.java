// File: Competitor
// Description: to show all competitor score
...
public class Competitor {
	...
	public void setScore(double jScore, double[] sScore) {
		//TODO: Code HERE
		// สร้าง Object Score ใหม่เพื่อเก็บคะแนนในแต่ละรอบ
		Score newScore = new Score();
		newScore.setScores(jScore);
	    newScore.setScores(sScore);
	    this.score.add(newScore); // เพิ่มลงใน List เพื่อรองรับการแข่งขันหลายรอบ
	}
	...
	public void showRawScore(int round) {
		//TODO: Code HERE
		Score s = this.score.get(round - 1);
	    System.out.print("Jumping score: " + getJScore(round));
	    System.out.print("\nAriel Style (Raw) score: ");
	    for (double val : s.getStyleScore()) {
    ...
    */
	public double getJScore(int round) {
		//TODO: Code HERE
		return (this.score.get(round - 1).getJumpScores() / maxKpoint) * 10;
	}
    ...
    */
	public double getFinalScore(int round) {
		//TODO: Code HERE
		double jScore = getJScore(round); // คะแนนระยะกระโดด
		double sScore = this.score.get(round - 1).getCalibratedAverage(); // คะแนนสไตล์
