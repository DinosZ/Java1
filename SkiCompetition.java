...
public class SkiCompetition {
    ...
    */
	public List<Competitor> getGoldMedal(List<Competitor> competitors, int round){
		//TODO: Code HERE
		List<Competitor> qualified = new ArrayList<>();
		// 1. คัดกรองนักกีฬาที่คะแนนถึงเกณฑ์ 8.0
		for (Competitor c : competitors) {
	        if (c.getFinalScore(round) > 8.0) {
	            qualified.add(c);
	        }
	    }
		// 2. เรียงลำดับคะแนนจากมากไปน้อย (Descending Order) [cite: 74]
	    qualified.sort((c1, c2) -> 
	        Double.compare(c2.getFinalScore(round), c1.getFinalScore(round)));

	    // 3. เลือกเฉพาะ 3 อันดับแรก [cite: 74]
	    if (qualified.size() > 3) {
	        return qualified.subList(0, 3);
	    }
	    return qualified;
	}
